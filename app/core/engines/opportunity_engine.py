...
class OpportunityEngine:
    ...
    def get_market_data(self, ticker: str) -> Optional[Dict]:
        """Get latest price for ticker (placeholder - would use vnstock in production)"""
        # Placeholder: In real implementation, fetch from vnstock
        # For now return mock
        return {"close": 100000}  # Mock

    def run_opportunity_scan(self) -> Dict[str, Any]:
        """Full scan cycle: find events, analyze impact, generate proposals"""
        print(f"[{datetime.utcnow()}] Starting opportunity scan...")
        
        tickers = self.get_watchlist_tickers()
        print(f"  Watching {len(tickers)} tickers: {tickers}")
        ...
        print(f"  {len(relevant_events)} events affect watched universe")
        ...
        for ticker, data in ticker_events.items():
            try:
                ...
                print(f"  Error generating proposal for {ticker}: {e}")
    ...
    def _generate_proposal_for_ticker(self, ticker: str, data: Dict) -> Optional[Proposal]:
        ...
        if recent and recent.status == "ACTIVE":
            print(f"  {ticker}: Already has ACTIVE proposal v{recent.version}, skipping")
            return None
        ...
        if not fund_holdings:
            print(f"  {ticker}: Not in fund universe, skipping")
            return None
        
        # Build proposal using LLM
        proposal_data = self.llm.generate_proposal(
            ticker=ticker,
            current_price=market_data.get("close", 100000) if market_data else 100000,
            holdings_info=fund_holdings,
            recent_events=data["events"],
            fundamentals={},  # Would fetch in production
        )
        ...
        proposal = Proposal(
            ...
            valid_until=datetime.utcnow() + timedelta(days=proposal_data.get("horizon_days", 365)),
